...
Calculates druggability_score and pocket_volume for GPCR structures
...
def parse_fpocket_results(fpocket_output_dir):
    """Parse fpocket output to extract druggability_score and pocket_volume."""
    ...
    # Return best pocket (highest druggability_score)
    best_idx = druggability_scores.index(max(druggability_scores))
...
def main():
    ...
    for _, row in targets.iterrows():
        ...
        if success:
            druggability_score, pocket_volume, n_pockets = parse_fpocket_results(target_out)
            results.append({
                'UniProt': uniprot,
                'Target': target,
            ...
            print(f"    druggability_score={druggability_score}, n_pockets={n_pockets}")
